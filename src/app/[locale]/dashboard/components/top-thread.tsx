...
export async function TopThread({ group, ...props }: { group: string }) {
  const t = await getTranslations();
  let task = await getKolTop(group, "24h", true);
            ...
            {group === "cn" ? (
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>{t("dashboard.topThreadCnHelp")}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ) : null}
          </div>
          <Link href={`/twitterFeeds/tweets?group=${group}&isThread=true`}>
